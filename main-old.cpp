...
void usage()
{
	cerr<<"pbmt -extractPhrase -src=[sourcefile] -tar=[targetfile] -align=[alignment] [-s2tLex=file] [-t2sLex=file] [-w=weightfile]"<<endl
		<<"     [-normalize=true|false] [-reverse] [-srcLengthLimit=int] [-tarLengthLimit=int] [-maxNumOfUnAlignedWords=int]"<<endl
		<<"     [-printTopEntrys=int] [-onlyonefeat] [-threshold=double] [moore=bool]"<<endl
		<<"pbmt -makeLexDic -src=[sourcefile] -tar=[targetfile] -align=[alignment] [-w=weightfile] [-s2t=filename] [-t2s=filename]"<<endl
		<<"pbmt -decode -pt=[phrase table file] -dmax=[max distortion limit] -beam=[beam size] -weights=[d:d:d...]"<<endl
		<<"pbmt -qsubMFT -curDir=dir -stepLength=int -nStep=int -dConfig=trainConfig -o=ruleFile [-walltime=time] [-pmem=mem] [-queue=node]"<<endl
		<<"pbmt -rule_refinement -i=raw_rules"<<endl
		<<"pbmt -reprintLexDic -i=file"<<endl
		;
	exit(1);
}
...
void usage()
{
	cerr<<"decoder --pt [phrase table file] --dmax [max distortion limit] --beam [beam size] --weights [d:d:d...]"<<endl;
	exit(1);
}
*/
