    ...
    for(auto& sp: cache){
                ...
                for(int i=0;i<sp.n;i++){
                    for(int ilen=0;ilen<sp.l;ilen++){
                        if(sp(i,ilen,j,jlen)!=(void*)0){
                            target_probs[j][jlen]+=sp(i,ilen,j,jlen)->prob;
                        }
                    }
                }
        ...
        for(int j=0;j<sp.m;j++){
            for(int jlen=0;jlen<sp.l&&j+jlen+1<=sp.m;jlen++){
                double segprob=0;
                if(j==0&&j+jlen+1==sp.m)segprob=1.0/backward[0];
                else if(j==0)segprob=backward[j+jlen+1]/backward[0];
                else if(j+jlen+1==sp.m)segprob=forward[j]/backward[0];
                else segprob=forward[j]*backward[j+jlen+1]/backward[0];
                for(int i=0;i<sp.n;i++){
                    for(int ilen=0;ilen<sp.l;ilen++){
                        if(sp(i,ilen,j,jlen)!=(void*)0){
